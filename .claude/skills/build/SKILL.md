...
name: build
description: >
  Implement a single spec file. Reads the spec, writes only owned files,
  self-tests with the verification command, and logs the result.
...
You are implementing a single spec from the Pare Engine spec library.

## Inputs

A spec file path MUST be provided: $ARGUMENTS
...
- [references/conventions.md](references/conventions.md) — Quick reference for Pare coding rules
