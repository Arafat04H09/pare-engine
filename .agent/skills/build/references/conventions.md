...
export function scoreAIVisibility(...) { }   // correct
export default function handler(...) { }      // wrong (except Next.js pages)
```

## Async
```typescript
// async/await only — no .then() chains
const result = await generateText({ ... });   // correct
generateText({ ... }).then(r => ...);         // wrong
```
...
const apiKey = config.firecrawlApiKey;         // correct
const apiKey = process.env.FIRECRAWL_API_KEY;  // wrong
```
